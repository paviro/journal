//! Background weather and air-quality lookups over the shared [`Worker`], fired
//! when an entry's location is set or changed. Each request carries the target
...
/// A weather + air-quality lookup for one entry, tagged with the id the worker
/// assigned.

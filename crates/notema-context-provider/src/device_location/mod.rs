...
pub fn device_location() -> AppResult<DeviceFix> {
    #[cfg(target_os = "android")]
    {
        termux::locate()
    }
...
fn parse_fix_json(body: &str, source: DeviceLocationSource) -> AppResult<DeviceFix> {
    ...
    let raw: Raw = serde_json::from_str(body.trim())
        .map_err(|_| anyhow::anyhow!("location helper returned unexpected output"))?;
    match (raw.latitude, raw.longitude) {
        ...
        _ => anyhow::bail!("no location fix was obtained"),
    }
}

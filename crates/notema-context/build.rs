...
//! `notema-locate` is wrapped in a signed `.app`, zipped, and run at runtime.
//!
...
fn main() {
    ...
    let workspace = manifest_dir
        ...
        .unwrap()
        .to_path_buf();
    let locate_crate = workspace.join("crates/notema-locate");
    let plist_src = locate_crate.join("NotemaLocate-Info.plist");
    let entitlements = locate_crate.join("NotemaLocate.entitlements");
    ...
    let zip = out_dir.join("NotemaLocate.app.zip");
    ditto(&["-c", "-k", "--keepParent"], &app, &zip);
    println!("cargo:rustc-env=NOTEMA_LOCATE_HELPER_ZIP={}", zip.display());
}
...
fn build_helper(target: &str, out_dir: &Path) -> PathBuf {
    ...
    run(
        Command::new(cargo)
            ...
            .arg("--target-dir")
            .arg(&target_dir),
        "build the notema-locate helper",
    );
    target_dir.join(target).join("release/notema-locate")
}
...
fn assemble_bundle(binary: &Path, plist_src: &Path, out_dir: &Path, version: &str) -> PathBuf {
    let app = out_dir.join("NotemaLocate.app");
    let _ = fs::remove_dir_all(&app);
    let macos_dir = app.join("Contents/MacOS");
    fs::create_dir_all(&macos_dir).expect("create bundle MacOS dir");
    fs::copy(binary, macos_dir.join("notema-locate")).expect("copy helper binary");

...
//! prompt. So the actual CoreLocation code lives in the tiny `journal-locate`
...
/// The signed `JournalLocate.app`, zipped (built by this crate's `build.rs`).
...
fn ensure_helper() -> AppResult<PathBuf> {
    let dir = support_dir()?;
    let app = dir.join("JournalLocate.app");
    let binary = app.join("Contents/MacOS/journal-locate");
    ...
    let zip = dir.join("JournalLocate.app.zip");
